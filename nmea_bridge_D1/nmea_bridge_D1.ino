...
//extra RGB LED indicator
//Red - failure alarm
//Blue - wifi status
//Green - USB message activity
#define RGB_LED                 1   //comment this line if you don't have one
#ifdef RGB_LED
  #define RGB_LED_PIN           D2  // where RGB led connected
#endif
...
struct Configuration default_config = {
    ...
    // rx_baudrate (4800)
    1,
    // brightness of LED indicator if used
    100,
};
...
bool inConfigMode = false;
  ...
  uint8_t led_state_g = 0;
  uint8_t led_state_b = 0;
  struct CRGB leds[1];  //we have 1-led strip
  #define rgbled leds[0];
#endif
...
/* USB Part 
*/
uint8_t usb_task_state = USB_STATE_DETACHED;
...
/*
  ...
  if (rcode) {
    force_alarm_led_on();
    DEBUG_PRINT("GetModemStatus");
    DEBUG_PRINTLN("rcode");
  }

  DEBUG_PRINT("mcr: ");
  DEBUG_PRINTLN(lc.msr, HEX);
...
unsigned int  init_timer=0;  //timer to reInit USB

#define MAX_PCKT_SIZE 32  //TODO USB packet size. This value must be received from USB device!
uint8_t buf[MAX_PCKT_SIZE+1]; 
char usb_sentence_buffer[(NMEA_SENTENCE_BUFFER_SIZE + 1)*2];
uint8_t usb_sentence_buffer_filled = 0;  // number of chars in buffer
...
void setup() {
    ...
    fix_config();
      ...
      LEDS.setBrightness(config.led_brightness); // limit maximum brightness
      LEDS.addLeds<WS2812B, RGB_LED_PIN, GRB>(leds, 1); // settings for our strip (strip on WS2811, WS2812, WS2812B)
      //show rainbow at start
      for (int ihue = 0 ; ihue < 255; ihue++ ) {
        leds[0] = CHSV(ihue, 255, 255);
        LEDS.show();
        delay(20);
      }
      leds[0] = CHSV(0, 0, 0);
      force_alarm_led_on(); // Fire alarm while startup
    #endif
    ...
    if (Usb.Init() == -1)  {  //try to Init USB device
    ...
    if (effective_wifi_mode == WifiMode::access_point) {
        ...
        //WiFi.softAP(effective_wifi_ssid, "1234567890", WIFI_ACCESS_POINT_CHANNEL, WIFI_ACCESS_POINT_MAX_CONNECTIONS);
    ...
    DEBUG_PRINTLN("/");
    DEBUG_PRINT("PWD: ");
    DEBUG_PRINTLN(config.wifi_password);
...
void handle_usb_event() {
  ...
  usb_task_state = Usb.getUsbTaskState();
  if ( usb_task_state != USB_STATE_RUNNING ) {
    force_alarm_led_on();
    DEBUG_PRINTLN(Usb.getUsbTaskState());
    if (init_timer==0)  { init_timer = millis(); } else {
      if (millis()-init_timer>5000) {
        DEBUG_PRINTLN("reinit");
        init_timer=0;
        if (Usb.Init() == -1)
          DEBUG_PRINTLN("OSC did not start.");
        }
  ...
  if (ch340.isReady()) {
    ...
    uint16_t rcvd = MAX_PCKT_SIZE;
    while (rcvd>0) {
      for (uint8_t i = 0; i < MAX_PCKT_SIZE+1; i++)
        buf[i] = 0;
      uint16_t rcvd = MAX_PCKT_SIZE;
      rcode = ch340.RcvData(&rcvd, buf);
      if (rcode && rcode != hrNAK) {
        DEBUG_PRINT("Ret ");  DEBUG_PRINTLN(rcode);
        force_alarm_led_on();   
      }
      if (rcvd > 0) {
        //DEBUG_PRINTLN((char*)(buf));
        strncpy(usb_sentence_buffer + usb_sentence_buffer_filled, (char*)buf, rcvd);
        usb_sentence_buffer_filled +=rcvd;
        if (usb_sentence_buffer_filled >= NMEA_SENTENCE_BUFFER_SIZE) {
            DEBUG_PRINTLN("usb sentence buffer overflow");
            ...
            usb_sentence_buffer[NMEA_SENTENCE_BUFFER_SIZE] = '\n';
            usb_sentence_buffer_filled = NMEA_SENTENCE_BUFFER_SIZE + 1;
        }
        if (usb_sentence_buffer[usb_sentence_buffer_filled-1] == '\n') {
          force_msg_led_on(); 
          handle_outgoing_sentence( usb_sentence_buffer, usb_sentence_buffer_filled );
          transmit_incoming_over_serial( usb_sentence_buffer, usb_sentence_buffer_filled );
          //DEBUG_PRINT(usb_sentence_buffer);
          usb_sentence_buffer_filled = 0;
          for (uint8_t i = 0; i < (NMEA_SENTENCE_BUFFER_SIZE + 1)*2; i++)
              usb_sentence_buffer[i] = 0;
          break;
        }
        delay(5);
      } else { break; }
...
void send_info_page_response() {
    ...
    */
    ...
    write_info_html_section_start("USB");
    write_info_html_field(
        "USB task state",
        (usb_task_state == USB_STATE_RUNNING ? String("Running") : String(usb_task_state)));
    write_info_html_section_end();
...
void send_config_form_response() {
    ...
    */
    ...
    write_form_html_heading("Device");
      ...
      write_form_html_select_end();
    #endif
    write_form_html_field("number", "l", String(config.led_brightness), "LED brightness", "", " min=\"0\" max=\"255\" required");
    ...
    for (uint8_t i = 0; i < http_server.args(); i++) {
        ...
        switch (argname_str[0]) {
            ...
            case '9':
                ...
                break;
            case 'l':
                brightness = atoi(argvalue_str);
                if (brightness == new_config.led_brightness) {
                    continue;
                }
                if ((0 < brightness) && (brightness <= 255)) {
                    new_config.led_brightness = brightness;
...
void fix_config() {
    ...
    */
    ...
    if (!((0 < config.led_brightness) && (config.led_brightness <= 255))) {
        config.led_brightness = default_config.led_brightness;
        config_changed = true;
    }
